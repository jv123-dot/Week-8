 ...
 *
 * ↓ YOUR CODE HERE ↓ */
let myGreeting = new Greeting("J", "ID")
myGreeting.hello()
/*-------------------------------------------------------*/
// Question 3: myBook
console.log(`--------------------------
...
let yourBook = new Book("LoTR", "J. R. R. Tolkien");
 ...
 * ↓ YOUR CODE HERE ↓ */
class Fruit {
  constructor(name, size, shape, color, taste) {
    this.name = name
    this.size = size
    this.shape = shape
    this.color = color
  ...
  describe() {
    console.log(`This fruit is a ${this.name}, its size is ${this.size}, its shape is ${this.shape}, its color is ${this.color} and finally it tastes ${this.taste}.`)
  }
}

let fruit1 = new Fruit("Lime,", "about the size of a tennis ball", "oblong", "green", "sour");
let fruit2 = new Fruit("Banana", "the size of a typical banana", "like that of a banana", "yellow", "sweet");
fruit1.describe();
fruit2.describe();
...
class List {
  ...
  displayMovies() {
    ...
    for (let i = 0; i < this.movies.length; ++i) {
      movieInfo += `${this.movies[i].title}, 
      directed by ${this.movies[i].director}\n`;
    } 
